  ...
  matches.forEach((matchIndex) => {
    ...
    parts.push(
      <span
        ...
        className='rounded bg-blue-500/30 px-0.5 font-bold text-blue-300'
      >
        {text[matchIndex]}
      </span>
    )
...
// Memoized components
const ExecutionCard = memo(
  ({ execution, isSelected, onClick, searchQuery }) => {
    const StatusIcon =
...
const LogEntry = memo(({ log, isLast }) => {
  ...
  return (
    <div className='relative pl-8'>
      {!isLast && (
        <div className='absolute top-6 bottom-0 left-2 w-0.5 bg-gradient-to-b from-blue-600/40 to-transparent' />
      )}
      <div
        className={`absolute top-1 left-0 rounded-full p-0.5 ${statusColor}`}
      >
      ...
      <div className='group rounded-lg border border-neutral-700/50 bg-gradient-to-br from-neutral-800/50 to-neutral-900/30 p-4 transition-all hover:border-blue-600/30 hover:shadow-md hover:shadow-blue-500/10'>
        ...
        {log.output_data && (
          <details className='mt-3'>
            <summary className='flex cursor-pointer items-center gap-2 text-xs text-neutral-400 hover:text-blue-400'>
              <span className='font-medium'>View Output</span>
              <span className='text-neutral-600'>▼</span>
            </summary>
            <pre className='custom-scrollbar mt-2 max-h-48 overflow-auto rounded-md border border-neutral-800/50 bg-neutral-950/70 p-3 text-xs text-neutral-300'>
...
export default function WorkflowExecutionsPage() {
  ...
  const {
    ...
    refetch
  } = useWorkflowExecutions(id)

  // Fetch execution details when selected
  const { data: executionDetails, isLoading: logsLoading } =
    useWorkflowExecutionDetails(id, selectedExecutionId)
  ...
  const filteredExecutions = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((execution) => {
        ...
        for (
          let i = 0;
          i < idLower.length && searchIndex < searchLower.length;
          i++
        ) {
  ...
  return (
    <>
      ...
      <SideBarLayout userProfile={userProfile}>
        ...
        <div className='custom-scrollbar ove rflow-y-auto flex-1'>
                ...
                <Card className='border-blue-600/20 bg-gradient-to-br from-blue-900/20 to-blue-950/10 shadow-xl'>
                  <div className='mb-6 space-y-4'>
                    ...
                    <p className='text-sm text-neutral-400'>
                      Showing {paginatedExecutions.length} of{' '}
                  ...
                  {filteredExecutions.length === 0 ? (
                    <div className='flex flex-col items-center justify-center py-16'>
                      <Clock className='mb-4 h-16 w-16 text-blue-400' />
                ...
                {selectedExecution ? (
                  <div className='flex h-full flex-col'>
                    ...
                    <div className='flex-1 overflow-hidden'>
                      <h3 className='mb-4 flex items-center gap-2 font-semibold text-orange-300'>
                  ...
                  <div className='flex h-full flex-col items-center justify-center py-16'>
                    <Activity className='mb-4 h-20 w-20 text-orange-400' />
