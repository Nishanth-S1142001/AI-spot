...
export default function Home() {
  // State for Sidebar
  const [isSidebarOpen, setIsSidebarOpen] = useState(true) // Promoted state from HomeSidebar
    ...
    if (rEmailValidation()) {
      ...
      try {
        ...
        const { data, error } = await supabase.auth.resetPasswordForEmail(
          rEmail,
          {
            orangeirectTo: window.location.origin + '/auth/update-password'
          }
        )
  ...
  const onRegisterSubmit = async (e) => {
    ...
    // NOTE: Supabase only requires 6 chars, but the local check requires 8. Adjusting local check for consistency with the one above, but for a real app, it should be 8+.
    if (formData.password.length < 6) {
      setRegisterError('Password must be at least 6 characters long.')
      return
    }

    // Original password regex was too restrictive for a simple example:
    ...
    try {
      ...
      setFormData({
        fullName: '', // Typo fixed from firstName/lastName to fullName
        email: '',
        password: '',
        cpassword: ''
      })
  ...
  // Adjusted: Removed unused formData fields and kept full_name consistent
    ...
    <div className='flex h-screen flex-col overflow-x-hidden font-mono sm:flex-row'>
      ...
      <div
        className={`fixed inset-0 z-20 transition-all duration-500 ${
          isOpen ? 'backdrop-blur-sm' : 'pointer-events-none' // pointer-events-none ensures you can still click things when the modal is closed
        }`}
      />
      {/* 1. Sidebar */}
      <HomeSidebar
      ...
      >
        ...
        - fixed inset-x-0 to pin horizontally
      ...
      */}
        ...
        to account for the fixed height of the navigation bar 
      */}
        <div
          id='home-section-container'
          className='flex min-h-screen flex-col pt-20' // Added pt-20 (or size of your navbar height)
        >
          ...
          >
            ...
            {/* Your existing hero content structure */}
          ...
          <main className='bg-opacity-80 relative z-10 text-white'>
            ...
            <section id='about' className='bg-neutral-900 px-4 py-20'>
              <div className='container mx-auto max-w-6xl'>
                <h2 className='mb-6 text-4xl font-extrabold text-neutral-100'>
                  About <span className='text-orange-600'> Us</span>
                </h2>
                <p className='text-lg leading-relaxed text-neutral-400'>
                  We are pioneers in the no-code AI revolution, empowering
                  businesses and individuals to harness the power of artificial
                  intelligence without technical barriers. Our platform enables
                  anyone to create intelligent AI agents that can transform the
                  ...
                  {[
                    {
                      ...
                      desc: 'An intuitive drag-and-drop interface to create bespoke AI agents tailoorange to your unique needs.'
                    },
                    {
                      icon: (
                        <MessageSquare
                    ...
                    },
                    {
                      ...
                      desc: 'Gain deep insights into your data through AI-poweorange analytics and reporting.'
                    },
                    {
                      icon: (
                        <Activity
            ...
            <section
              id='testimonials'
              className='h-fit bg-neutral-900 px-4 py-20'
            >
              <div className='container mx-auto max-w-6xl'>
                <h2 className='mb-16 text-left text-5xl font-extrabold text-neutral-100'>
                  Testimonials
                ...
                <div className='grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3'>
                  {[
                    ...
                    {
                      quote:
                        'The no-code interface is incorangeibly intuitive. Our productivity has skyrocketed since adopting it.',
