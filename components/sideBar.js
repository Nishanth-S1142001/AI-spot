/**
 * UNIFIED SIDEBAR SYSTEM - FINAL VERSION
 * Claude-style sidebar with user profile, main menu, and collapsible bottom section
 * Integrates seamlessly with existing SubSidebar and SideBarLayout
 */
...
export default function Sidebar({
  ...
  const renderMenuItem = useCallback(
    (item, idx, isBottomSection = false) => {
      const active = isActive(item)
      const hasSubmenu = item.submenu && item.submenu.length > 0
      const itemKey = isBottomSection
        ? `bottom-${item.key || idx}`
        : item.key || idx
      ...
      if (hasSubmenu) {
        return (
          <div key={itemKey} className='mb-1 px-2'>
            <button
    ...
    [isSidebarOpen, openSubmenu, pathname, isActive, toggleSubmenu, onSelect]
  )
  ...
  return (
    ...
    >
      ...
      <nav className='custom-scrollbar flex-1 overflow-y-auto py-4'>
        {mainMenuItems.map((item, idx) => renderMenuItem(item, idx, false))}
      </nav>
      ...
      {bottomMenuItems.length > 0 && (
        <div className='border-t border-neutral-800/50'>
          ...
          >
            ...
            >
              {/* Avatar */}
              <div className='flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-orange-500 to-orange-600 text-sm font-semibold text-white'>
                {user.avatar ? (
                  <img
                    src={user.avatar}
                    alt={user.name}
